"""
Shared trajectory tables under ``data/<run_slug>/`` plus table lookup at train/eval time.
One-step reset banks are shared across all experiment horizons T for the same system config.
"""
...
def data_slug(cfg: SBOEDConfig) -> str:
    """Shared data key per run slug (T-independent one-step bank)."""
    return cfg.run_slug
...
def data_dir_candidates(project_root: Path, cfg: SBOEDConfig) -> list[Path]:
    ...
    add(data_slug(cfg))
    add(legacy_data_slug(cfg))
    add(legacy_horizon_data_slug(cfg))
    add(f"{cfg.name}_T{cfg.step_number}")
    ...
    return [root / slug for slug in slugs]


def resolve_data_path(project_root: Path, cfg: SBOEDConfig) -> Path:
    """Use an existing data folder (new or legacy name), else the canonical new path."""
    for candidate in data_dir_candidates(project_root, cfg):
        if candidate.is_dir() and any(candidate.iterdir()):
            return candidate
    return data_dir(project_root, cfg)
