...
def generate_control_bank_for_split(
    ...
    *,
    ...
    # u_max safety for every particle
    u_max_safe = safe[:, -1].copy() if np.isclose(cands[-1], spec.u_max) else None
    if u_max_safe is None:
        r_max, n_max = engine.simulate_metrics_batch(
            M_rows, K_rows, np.full(n, spec.u_max), batch_size=batch_size,
        )
        u_max_safe = (r_max <= spec.rocof_limit_hz_s) & (n_max >= spec.delta_f_nadir_hz)

    report = {
        "n_systems": n,
        "n_candidates": n_c,
        "u_candidates": [float(x) for x in cands],
        "n_infeasible_u_req": int(n_infeasible),
