...
from src.objectives.eig.plots import find_latest_experiment_dirs
from src.reporting.run_context import load_experiment_run, model_dir
from src.domains.swing.design import build_catalog
from src.inference.scoring import TableThetaSupport

from src.objectives.eig.stepwise.evaluate import (
    STEPWISE_METHODS,
    _noise_seed,
...
from src.objectives.eig.stepwise.io import (
...
from src.reporting.summary import write_eig_summary_md
from src.objectives.eig.stepwise.plots import (
    plot_step1_heatmap,
    plot_stepwise_eig,
    ...
    for method in ("dad_eig", "rl_sboed_eig"):
        ...
        if not policy.is_file():
            raise FileNotFoundError(
                ...
                "Run: ./scripts/training.sh --method dad --exp-dir <experiment>"
            )
...
def write_system_outputs(
    ...
    *,
    ...
    write_eig_summary_md(
        exp_dir,
        system=str(payload["system_label"]),
        horizon=horizon,
        method_results=methods,
        meta={
            "T": horizon,
            "N_obs": payload.get("N_obs", payload.get("n_obs", 0)),
            "observation_mode": payload.get("observation_mode"),
            "N_sim": payload.get("N_sim", payload.get("n_sim")),
        },
    )
...
def run_system_stepwise_eig(
    ...
    *,
    ...
    if exp_dir is not None:
        # Explicit result dir (stamped eig_based folders need not match prefix globs).
        primary = exp_dir.resolve()
        if not primary.is_dir():
            raise FileNotFoundError(f"Experiment directory not found: {primary}")
    else:
        if not by_T:
            raise FileNotFoundError(
