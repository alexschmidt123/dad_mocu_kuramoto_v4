...
from src.objectives.eig.stepwise.evaluate import STEPWISE_METHODS
...
def write_report_md(
    ...
    *,
    ...
    for key in ("dad_eig", "rl_sboed_eig", "myopic_delta_h"):
    ...
    if {"dad_eig", "myopic_delta_h"}.issubset(method_results):
        dad = method_results["dad_eig"]
        ...
        if s1_dad < s1_my and term_dad > term_my:
            lines.append(
                "DAD-EIG accepts a **smaller Step-1 EIG** than Myopic ΔH but achieves "
                f"**larger terminal EIG** ({term_dad:.4f} vs {term_my:.4f} nats)."
            )
        elif term_dad > term_my:
            lines.append(
                f"DAD-EIG terminal EIG ({term_dad:.4f}) exceeds Myopic ΔH ({term_my:.4f})."
            )
        else:
            lines.append(
                f"Myopic ΔH terminal EIG ({term_my:.4f}) meets or exceeds DAD-EIG ({term_dad:.4f})."
            )
        if len(dad["mean_eig_by_step"]) > 1:
            ...
            lines.append(
                f"Mean later-step EIG (t≥2): DAD-EIG={later_dad:.4f}, Myopic ΔH={later_my:.4f}."
            )
