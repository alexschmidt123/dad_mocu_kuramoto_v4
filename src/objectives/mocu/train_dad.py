"""MOCU / objective-based DAD: REINFORCE minimizing terminal posterior-safe u_ctrl.

Policy nets live in ``src.policies``; this module is the table-bank Foster-style
trainer. The main Plan-2 / continuous MOCU trainer is ``src.objectives.mocu.train``.
"""
...
from src.banks.tables import lookup_action_y, validate_trajectory_y_sim
from src.objectives.eig.dad_rollout import (
    feasible_mask as _feasible_mask,
    policy_rollout as _policy_rollout,
    training_horizon as _training_horizon,
)
...
def train_dad_policy(
    ...
    *,
    ...
    # CSV loss record (required result-folder layout: model/*_training_metrics.csv)
    import csv

    csv_path = output_dir / f"{run_tag}_training_metrics.csv"
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        ...
        for i in range(n_ep):
            writer.writerow(
                {
                    "epoch": i + 1,
                    "loss": float(epoch_losses[i]),
                    "mean_train_u_ctrl": float(epoch_cost[i]),
                    "validation_mean_u_ctrl": (
                        float(epoch_val[i]) if i < len(epoch_val) else ""
                    ),
                    "policy_entropy": (
                        float(epoch_entropy[i]) if i < len(epoch_entropy) else ""
                    ),
                    "gradient_norm": (
                        float(epoch_grad_norm[i]) if i < len(epoch_grad_norm) else ""
                    ),
                }
            )
    import shutil
    ...
    if run_tag == "dad":
        shutil.copy2(csv_path, output_dir / "dad_training_metrics.csv")
    ...
    ax.plot(range(1, len(epoch_cost) + 1), epoch_cost, "g-o", markersize=3, label="train")
    if epoch_val:
        ax.plot(
            range(1, len(epoch_val) + 1),
            epoch_val,
            "b-s",
            markersize=3,
            label="val",
