...
* **Inertia ($M$):** $[0.01,\, 0.06]$ $s^2/\mathrm{rad}$. This corresponds to $H \in [2.3,\, 5.0]$ s via $M = 2H/\omega_s$ at $\omega_s = 2\pi\times 60$ rad/s (Kundur, 1994; typical synchronous machine range).
...
$$
* **$\sigma_{feat}$:** 0.05 Hz/s (aggregate uncertainty from numerical error and PMU noise; NASPI, 2021).
