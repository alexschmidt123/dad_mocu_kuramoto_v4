...
The system topology is fixed and defined by the standard IEEE 14-bus test case. We assume **homogeneous** uncertain parameters: $M_i = M$ and $K_i = K$ for all buses $i$, so the latent parameter vector is $\vartheta = (M, K)$ (scalar $M$, $K$).
...
![IEEE 14-bus network topology. Generated by `tests/test_experiment_design_pipeline.py::test_ieee14_diagram_plot`.](images/ieee14_diagram.png)
...
The (homogeneous) parameter vector $\vartheta = (M, K)$ is drawn from a uniform prior $p(\vartheta)$ over physically validated ranges for a 60 Hz system. **$M$ is the effective inertia coefficient** in the swing equation, related to the inertia constant $H$ (seconds) by $M = 2H/\omega_s$ with $\omega_s = 2\pi f_0$.
