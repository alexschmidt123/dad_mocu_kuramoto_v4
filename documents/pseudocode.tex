...
The \emph{target} of the sequential design is to minimize expected \emph{terminal} MOCU at $t=T$ (i.e.\ $\mathrm{MOCU}(p_T)$). Belief index $t$ runs from $0$ (initial; $p_0$ is the prior, all methods share the same MOCU) to $T$ (terminal; $p_T$ after $T$ steps); designs are applied at steps $t=1,\dots,T$. The myopic rule is \emph{generally suboptimal} for terminal MOCU (it can coincide with the optimum only in degenerate cases, e.g.\ when designs provide independent information).
...
We use the same sequential design structure as in Foster et al.\ (2021): latent $\vartheta$, design $\xi$, observation $y$, belief update, and inner/outer sampling; our objective is expected terminal MOCU.
...
The \emph{design space} $\Xi$ is the set of single-step probe designs. Each design is $\xi=(b,A,T_p)\in\Xi$: bus $b\in\{1,\dots,N\}$, amplitude $A$, duration $T_p$. At step $t$ we choose $\xi_t\in\Xi$.
...
At step $t$, the belief prior to the observation is $p_{t-1}(\vartheta)$. Upon applying design $\xi_t$ and observing the scalar feature $y_t$, the belief is updated via Bayes' rule: $p_t(\vartheta)\propto p_{t-1}(\vartheta)\,p(y_t\mid\vartheta,\xi_t)$. Over the discrete support $\{\vartheta_{\ninner}\}_{\ninner=1}^{\Ninner}$, the theoretical update scales the prior probability mass of each candidate point by its corresponding likelihood $\mathcal{L}_{\ninner}^{(t)} = p(y_t\mid\vartheta_{\ninner},\xi_t)$. The unnormalized posterior weights and the normalization constant (evidence) are given by:
\[
...
\]
where the expectation is over the randomness in observations $y_1,\dots,y_T$ (and optionally $\vartheta_{\mathrm{true}}\sim p_0$), and $p_T$ is the belief after $T$ steps. We choose design sequences that reduce decision-relevant uncertainty by the end of the horizon; then $\hat{\gamma}(p_T)$ minimizes expected cost $J$. Per-step MOCU is computed for monitoring and for greedy heuristics but is \emph{not} the quantity being optimized.
...
The goal is to minimize expected \emph{terminal} MOCU $\mathbb{E}[\mathrm{MOCU}(p_T)]$. Algorithm~\ref{alg:dad_mocu_loop} implements one \emph{episode} (one run over $T$ steps): at $t=0$ belief is $p_0$ (prior) and all methods share the same MOCU; at each step $t=1,\dots,T$ we choose design $\xi_t$, observe $y_t$, update belief to $p_t$, and compute $\hat{\gamma}(p_t)$ and $\mathrm{MOCU}(p_t)$. The design step can be \emph{greedy} (minimize expected MOCU one step ahead; tractable but generally suboptimal for terminal MOCU) or \emph{amortized} (policy $\pi_\phi(\cdot\mid h_{t-1})$ trained for terminal or cumulative MOCU; Foster et al., 2021). \textbf{Inner/outer:} \emph{Outer} ($\Nouter$)= simulating possible future observations (fake $y$'s). \emph{Inner} ($\Ninner$)= updating the belief (posterior) over the $\Ninner$ points (inner index $\ninner=1,\dots,\Ninner$) for each of those observations. Myopic runs outer $\times$ inner \emph{online} each step (slow); DAD trains offline with horizon $T\times\Nouter\times\Ninner$, then online only the inner ($\Ninner$) is used once per step for the real observation (fast).
...
For a given $\vartheta$, $\gamma^\ast(\vartheta)$ is the smallest $\gamma\ge 0$ such that the closed-loop trajectory under reference probe $\xi_{\mathrm{ref}}$ and control $u^{\mathrm{ctrl}}=\gamma g_i\omega_i$ satisfies $|\dot f|\le r_{\max}$ and $f\ge f_{\min}$ on $[0,T_c]$. We assume the safe set is $[\gamma^\ast,\infty)$ (monotonicity in $\gamma$; see \S\ref{sec:limitations}). Algorithm~\ref{alg:gamma_star} finds $\gamma^\ast$ by bracketing then bisection on the safety predicate $\Safe(\gamma;\vartheta)$.
...
\small
\begin{algorithmic}[1]
\Require Parameter $\vartheta$; network $\mathcal{G}$; limits $(r_{\max}, f_{\min})$; horizon $T_c$; reference probe $\xi_{\mathrm{ref}}$; tolerances $\varepsilon, N_{\mathrm{expand}}$; caps $\gamma_{\max}, \gamma_{\mathrm{cap}}$.
\Ensure Minimal safe gain $\gamma^\ast(\vartheta)$, or $\gamma_{\mathrm{cap}}$ if unsolvable.
...
We state limitations and modeling choices explicitly; each subsection indicates scope and possible extensions.
